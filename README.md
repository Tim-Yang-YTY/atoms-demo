...
当任何错误发生时（超时、API 报错、网络断开等），系统不会显示原始错误信息，而是执行完整的降级流程：

```
错误发生 → 友好提示 → 自动创建工单 → 请求用户邮箱 → 关联工单与联系方式 → Dashboard 可追踪
```

**用户看到的**：
1. 友好的错误消息："抱歉出了问题，我们已经创建了一个工单"
2. 邮箱输入框（预填已有邮箱）
3. 提交后显示工单号（如 `TK-M1X2Y3-A4B5`）
4. 提示去 Dashboard → Tickets 追踪进度
